...
NvlinkHostNumaResult<bool> ParseStrictBool(const std::string& value) {
...
NvlinkHostNumaResult<std::vector<int>> ParseNodeCsv(
...
NvlinkHostNumaResult<size_t> CheckedLcm(size_t left, size_t right) {
    if (left == 0 || right == 0) {
...
class ProductionNvlinkHostNumaEnvironment final
    : public NvlinkHostNumaEnvironment {
   public:
...
NvlinkHostNumaResult<NvlinkHostNumaOptions> ParseNvlinkHostNumaOptions(
    ...
    auto enabled_it = config.find(CONFIG_KEY_ENABLE_NVLINK_HOST_NUMA);
    ...
    auto nodes_it = config.find(CONFIG_KEY_NVLINK_HOST_NUMA_NODES);
...
CreateProductionNvlinkHostNumaEnvironment() {
    return std::make_unique<ProductionNvlinkHostNumaEnvironment>();
}
