...
RealClient::RealClient() {
    ...
    prepare_egm_store_pool_runtime();
}
...
RealClient::~RealClient() {
    ...
    quarantine_egm_store_pool_on_destroy(static_cast<bool>(teardown));
}
...
tl::expected<void, ErrorCode> RealClient::setup_internal_with_egm_store_pool(
    ...
    if (enable_egm_store_pool) {
        const EgmStorePoolRuntimeDependencies default_dependencies;
        auto setup_result = setup_egm_store_pool(
            *egm_store_pool_options, global_segment_size, local_buffer_size,
            runtime_dependencies != nullptr ? *runtime_dependencies
        ...
        publish_client_buffer_allocator(client_buffer_allocator);
...
tl::expected<void, ErrorCode> RealClient::setup_internal(
    ...
    return setup_internal_impl(config, nullptr);
}
...
tl::expected<void, ErrorCode> RealClient::tearDownAll_internal() {
    ...
    if (retained_egm_store_pool_state) {
        if (!cleanup_egm_store_pool(false)) {
    ...
    publish_client_buffer_allocator(nullptr);
    port_binder_.reset();
...
tl::expected<int64_t, ErrorCode> RealClient::execute_ranged_read(
    ...
    if (src_offset == 0 && size == total_size) {
        ...
        if (replica.is_disk_replica()) {
            ...
            auto alloc_result = allocate_client_buffer(total_size);
            if (!alloc_result) {
    ...
    auto partial_disk_read =
        ...
        auto alloc_result = allocate_client_buffer(buf_size);
        if (!alloc_result) {
...
RealClient::batch_get_into_internal(const std::vector<std::string> &keys,
    ...
    if (!disk_operations.empty()) {
        ...
        for (size_t di = 0; di < disk_operations.size(); ++di) {
            ...
            auto alloc_result = allocate_client_buffer(op.total_size);
            if (!alloc_result) {
