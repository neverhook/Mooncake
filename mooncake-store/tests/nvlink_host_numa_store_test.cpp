...
class NvlinkHostNumaStoreTestPeer {
   public:
    ...
    static void SetMasterMountFailureHook(RealClient& client,
        ...
        client.nvlink_host_numa_master_mount_failure_for_test_ =
            std::move(hook);
    }
...
tl::expected<StoreHardwareContext, std::string> PrepareStoreHardware() {
    ...
    for (const char* incompatible_env :
        ...
        if (std::getenv(incompatible_env) != nullptr) {
            return tl::make_unexpected(
                ...
                " is set; HOST_NUMA V1 requires the legacy cross-node "
    ...
    auto nodes = DiscoverNvlinkHostNumaNodes(options, 1, *environment);
    if (!nodes) {
        return tl::make_unexpected(
    ...
    auto local_node = ResolveNvlinkHostNumaLocalNode(
    ...
    std::optional<NvlinkHostNumaPlan> selected_plan;
    ...
    for (int attempt = 0; attempt != 7; ++attempt) {
        ...
        auto plan =
            PlanNvlinkHostNumaCapacity(requested, granularities, max_mr_size);
        if (plan) {
...
ConfigDict HardwareConfig(const InProcMaster& master,
    ...
    return {{CONFIG_KEY_LOCAL_HOSTNAME, hostname},
            ...
            {CONFIG_KEY_ENABLE_NVLINK_HOST_NUMA, "true"},
...
TEST(NvlinkHostNumaStoreTest, EnabledConfigRejectsNonNvlinkBeforeSetup) {
    auto client = RealClient::create();
    ConfigDict config = MinimalConfig("tcp");
    config[CONFIG_KEY_ENABLE_NVLINK_HOST_NUMA] = "true";
...
TEST(NvlinkHostNumaStoreTest, EmptyPartialSetupCleanupIsIdempotent) {
    auto client = RealClient::create();
...
TEST(NvlinkHostNumaStoreTest,
    ...
    // This is the state CleanupNvlinkHostNuma deliberately retains when any
...
#if !defined(USE_MNNVL) || !defined(USE_CUDA)
TEST(NvlinkHostNumaStoreTest, UnsupportedBuildFailsWithoutAllocations) {
    auto client = RealClient::create();
...
TEST(NvlinkHostNumaStoreHardwareTest,
    ...
    ASSERT_TRUE(client->nvlink_host_numa_enabled_);
    ASSERT_FALSE(client->nvlink_host_numa_globals_.empty());
    ASSERT_TRUE(client->nvlink_host_numa_local_.has_value());
    EXPECT_EQ(client->nvlink_host_numa_globals_.size(),
              hardware->plan.chunks.size());
    for (const auto& record : client->nvlink_host_numa_globals_) {
...
TEST(NvlinkHostNumaStoreHardwareTest,
    ...
    NvlinkHostNumaStoreTestPeer::SetMasterMountFailureHook(
        *client,
         ...
         fail_nth](const Segment& segment) -> std::optional<ErrorCode> {
            ...
            if (*mount_calls == fail_nth) {
                *failed_global_base = reinterpret_cast<void*>(segment.base);
                if (client_ptr->nvlink_host_numa_local_) {
                    *local_base =
                        client_ptr->nvlink_host_numa_local_->allocation->base();
                }
                *te_descriptor_visible_before_failure =
                    NvlinkHostNumaStoreTestPeer::HasLocalTeBuffer(
    ...
    EXPECT_EQ(
        FindMetricValue(
            *metrics, "mooncake_nvlink_host_numa_initialization_failures_total",
            "stage=\"mount\""),
        1U);
    EXPECT_EQ(
        FindMetricValue(*metrics,
                        "mooncake_nvlink_host_numa_rollback_attempts_total"),
        1U);
    const auto rollback_failures = FindMetricValue(
        *metrics, "mooncake_nvlink_host_numa_rollback_failures_total");
