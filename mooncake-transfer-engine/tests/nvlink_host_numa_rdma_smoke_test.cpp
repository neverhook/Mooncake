...
bool SelectHostNumaNode(int& node, std::string& source, std::string& error) {
    ...
    if (override_node != nullptr && *override_node != '\0') {
        ...
        source = "MC_NVLINK_HOST_NUMA_TEST_NODE";
        return true;
    }
...
ibv_device* SelectRdmaDevice(const DeviceList& list, std::string& name,
    ...
    const char* requested = std::getenv("MC_NVLINK_HOST_NUMA_RDMA_DEVICE");
...
TEST(NvlinkHostNumaRdmaSmokeTest, VerbsCanRegisterProviderVmmAllocation) {
    bool strict = false;
    std::string prerequisite_error;
    ASSERT_TRUE(ParseStrictMode("MC_REQUIRE_NVLINK_HOST_NUMA_RDMA", strict,
                                prerequisite_error))
        << prerequisite_error;
