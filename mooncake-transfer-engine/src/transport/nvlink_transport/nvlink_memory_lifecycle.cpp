...
NvlinkTransport::NvlinkTransport()
    ...
    : use_fabric_mem_(supportsFabricMemory()),
      ...
      consumer_metrics_(createConsumerMetrics())
#else
    : use_fabric_mem_(supportsFabricMemory()),
      consumer_metrics_(createConsumerMetrics())
#endif
...
int NvlinkTransport::publishLocalRegistration(void* registration_addr,
    ...
    try {
        rc = addMetadataBuffer(descriptor, update_metadata);
    } catch (const std::exception& error) {
    ...
    if (rc != 0) {
        ...
        try {
            rollback_rc = removeMetadataBuffer(
...
int NvlinkTransport::addMetadataBuffer(const BufferDesc& descriptor,
                                       bool update_metadata) {
    if (test_dependencies_ && test_dependencies_->addBuffer) {
        return test_dependencies_->addBuffer(descriptor, update_metadata);
    }
    return metadata_->addLocalMemoryBuffer(descriptor, update_metadata);
}

int NvlinkTransport::removeMetadataBuffer(void* addr, bool update_metadata) {
    if (test_dependencies_ && test_dependencies_->removeBuffer) {
        return test_dependencies_->removeBuffer(addr, update_metadata);
    }
    return metadata_->removeLocalMemoryBuffer(addr, update_metadata);
}

std::shared_ptr<NvlinkTransport::SegmentDesc> NvlinkTransport::getSegmentDesc(
    uint64_t target_id) {
    if (test_dependencies_ && test_dependencies_->getSegment) {
        return test_dependencies_->getSegment(target_id);
    }
    return metadata_->getSegmentDescByID(target_id);
}
...
int NvlinkTransport::unregisterLocalMemory(void* addr, bool update_metadata) {
    ...
    if (registration.published) {
        ...
        try {
            rc =
                removeMetadataBuffer(registration.mapped_base, update_metadata);
        } catch (const std::exception& error) {
...
int NvlinkTransport::relocateSharedMemoryAddress(uint64_t& dest_addr,
                                                 uint64_t length,
                                                 uint64_t target_id) {
    auto desc = getSegmentDesc(target_id);
    if (!desc) {
