...
EgmStorePoolResult<void> ValidateEgmStorePoolConfiguration(
    const EgmStorePoolOptions& options, const std::string& protocol,
    size_t global_segment_size, size_t local_buffer_size) {
    if (!options.enabled) return {};
    if (protocol != "nvlink") {
        return tl::make_unexpected(
            "enable_egm_store_pool requires protocol=nvlink");
    }
    if (global_segment_size == 0) {
        return tl::make_unexpected(
            "enable_egm_store_pool requires a nonzero global_segment_size");
    }
    if (local_buffer_size != 0) {
        return tl::make_unexpected(
            "enable_egm_store_pool requires local_buffer_size=0; EGM local "
            "workspace support is out of V1 scope");
    }
    return {};
}
