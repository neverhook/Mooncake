...
class FakeAllocation final : public NvlinkHostNumaAllocation {
   public:
...
class FakeOperations final : public NvlinkHostNumaOperations {
   public:
    ...
    tl::expected<std::unique_ptr<NvlinkHostNumaAllocation>, ErrorCode> Allocate(
        ...
        const bool fail = Record(
            ...
            request.role == NvlinkHostNumaAllocationRole::kLocal ? "local"
                                                                 : "global");
        ...
        return std::unique_ptr<NvlinkHostNumaAllocation>(new FakeAllocation(
...
struct State {
    std::vector<NvlinkHostNumaGlobalRecord> globals;
    std::optional<NvlinkHostNumaLocalRecord> local;
    bool allocator_installed = false;
};

NvlinkHostNumaPlan PlanWithChunks(size_t chunk_count) {
    ...
    for (size_t index = 0; index < chunk_count; ++index) {
        plan.chunks.push_back(NvlinkHostNumaChunkPlan{
...
auto RunSetup(FakeOperations& operations, State& state,
              const NvlinkHostNumaPlan& plan, size_t local_size) {
    return SetupNvlinkHostNumaOrchestration(operations, plan, 0, local_size,
                                            state.globals, state.local,
                                            state.allocator_installed);
}

bool Cleanup(FakeOperations& operations, State& state) {
    return CleanupNvlinkHostNumaOrchestration(
...
TEST(NvlinkHostNumaOrchestratorTest,
    ...
    ASSERT_FALSE(setup);
    EXPECT_EQ(setup.error().error, ErrorCode::INTERNAL_ERROR);
    EXPECT_EQ(setup.error().stage, NvlinkHostNumaOrchestrationStage::kMount);
