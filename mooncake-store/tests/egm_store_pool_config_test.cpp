...
TEST(EgmStorePoolConfigTest, ValidatesGlobalOnlyV1Boundary) {
    EgmStorePoolOptions disabled;
    EXPECT_TRUE(ValidateEgmStorePoolConfiguration(disabled, "tcp", 0, 4096));

    EgmStorePoolOptions enabled;
    enabled.enabled = true;
    EXPECT_TRUE(ValidateEgmStorePoolConfiguration(enabled, "nvlink", 4096, 0));
    EXPECT_FALSE(ValidateEgmStorePoolConfiguration(enabled, "tcp", 4096, 0));
    EXPECT_FALSE(ValidateEgmStorePoolConfiguration(enabled, "nvlink", 0, 0));
    EXPECT_FALSE(
        ValidateEgmStorePoolConfiguration(enabled, "nvlink", 4096, 4096));
}
