...
class RealClient : public PyClient {
   public:
    ...
    bool nvlink_host_numa_enabled_ = false;
    std::vector<NvlinkHostNumaGlobalRecord> nvlink_host_numa_globals_;
    std::optional<NvlinkHostNumaLocalRecord> nvlink_host_numa_local_;

    tl::expected<void, ErrorCode> SetupNvlinkHostNuma(
        const NvlinkHostNumaOptions& options, size_t global_segment_size,
        size_t local_buffer_size);
    bool CleanupNvlinkHostNuma(bool rollback);
   ...
   private:
    ...
    bool nvlink_host_numa_allocator_installed_ = false;

    // Test-binary-only fault injection forwarded to Client immediately before
    // HOST_NUMA mount publication. Client invokes it only after the real TE
    // registration succeeds and before the Master RPC. It is deliberately
